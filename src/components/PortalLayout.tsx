...
export default function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  // Get user initials
  const getInitials = () => {
    if (!user?.user_metadata?.full_name) return "UF";
          ...
          <div className="p-4 border-t border-border">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="text-sm overflow-hidden">
                <div className="font-bold truncate">{user?.user_metadata?.full_name || "User"}</div>
                <div className="text-xs text-muted-foreground truncate">{user?.email}</div>
              </div>
            </div>
            <Button 
              variant="outline" 
              className="w-full rounded-none text-xs uppercase font-bold"
              onClick={logout}
            >
              Sign Out
            </Button>
